...
import { instructorTableHeader } from './instructor-table-header';
type UserStatusPayload = Omit<IUpdatedUser, 'id'>;

@Component({
  selector: 'toboggan-ws-instructor-list',
...
export class InstructorListComponent implements OnInit, OnDestroy {
  ...
  private filterFuncs: { [key: string]: ITableRowFilterFunc } = {
    status: (tr: TableRow, columnMetadata = instructorTableHeader) => {
      const isInvalid = !(
  ...
  activateUser(id: string, userPayload: UserStatusPayload) {
    this.modalAlertService.showModalAlert({
      type: 'warning',
      heading: 'Activate this instructor?',
  ...
  deactivateUser(id: string, userPayload: UserStatusPayload) {
    this.modalAlertService.showModalAlert({
      type: 'warning',
      heading: 'Deactivate this instructor?',
  ...
  private async toggleUserStatus(
    ...
    await this.associationGroupService.updateInstructorStatus(
      {
        status: status,
      },
      userId
  ...
  openRemoveUserConfirmation(user: any) {
    ...
    this.modalAlertService.showModalAlert({
      type: 'warning',
      heading: `Remove this instructor?`,
      message: `If you remove <strong>${userName}</strong>, they’ll no longer be associated wih the learners in this group. 
  ...
  async removeUser(user: any) {
    ...
    try {
      ...
      this.showNotification(
        'success',
        ``,
        `<strong>${userName}</strong> has been removed from <strong>Instructors</strong>.`,
        true
      );
    } catch (error) {
      this.showNotification(
        ...
        ` couldn’t be removed from <strong>Instructors</strong>.`,
        true,
        null
      );
    }
  ...
  private refreshTableData(
    ...
    this.dataGeneratorFactoryOutputObserver =
      this.tableDataService.dataGeneratorFactoryObs(
        this.associationGroupService.getInstructorsByGroup('1'), // for development purpose
